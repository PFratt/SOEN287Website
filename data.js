...
/*function formActionToATags(){
	var aTags = document.getElementsByTagName('a');
	for(let i = 0; i < aTags.length; i++){
		if(aTags[i].classList.contains("food-item") or aTags[i].classList.contains("food-name"))
			targetElements.push(aTags[i]);
	}
	for(let i = 0; i < targetElements.length; i++)
		targetElements[i].addEventListener('click',submitForm);
}
